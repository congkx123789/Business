import Foundation
import SwiftUI

// Reading Preferences model - CRITICAL for cross-device sync
struct ReadingPreferences: Codable {
    var fontSize: Int = 16
    var lineHeight: Double = 1.7
    var backgroundColor: String = "#0f172a"
    var textColor: String = "#e2e8f0"
    var fontFamily: String = "system"
    var readingMode: ReadingMode = .scroll
    var brightness: Double = 1.0 // 0.6 - 1.2
    var tapToToggleControls: Bool = true
    var autoHideControls: Bool = true
    var controlsTimeout: Int = 4000 // milliseconds
    var syncedAt: Date?
    
    enum ReadingMode: String, Codable {
        case scroll
        case pageTurn = "page-turn"
        case multiColumn = "multi-column"
    }
    
    enum CodingKeys: String, CodingKey {
        case fontSize = "font_size"
        case lineHeight = "line_height"
        case backgroundColor = "background_color"
        case textColor = "text_color"
        case fontFamily = "font_family"
        case readingMode = "reading_mode"
        case brightness
        case tapToToggleControls = "tap_to_toggle_controls"
        case autoHideControls = "auto_hide_controls"
        case controlsTimeout = "controls_timeout"
        case syncedAt = "synced_at"
    }
    
    // Convert to SwiftUI Color
    var backgroundColorUI: Color {
        Color(hex: backgroundColor) ?? Color.black
    }
    
    var textColorUI: Color {
        Color(hex: textColor) ?? Color.white
    }
}

extension Color {
    init?(hex: String) {
        let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int: UInt64 = 0
        Scanner(string: hex).scanHexInt64(&int)
        let a, r, g, b: UInt64
        switch hex.count {
        case 3: // RGB (12-bit)
            (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
        case 6: // RGB (24-bit)
            (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
        case 8: // ARGB (32-bit)
            (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
        default:
            return nil
        }
        self.init(
            .sRGB,
            red: Double(r) / 255,
            green: Double(g) / 255,
            blue: Double(b) / 255,
            opacity: Double(a) / 255
        )
    }
}

